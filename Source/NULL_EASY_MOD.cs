    ...
    public class Class1
    {
        public class Harmony_Patch
        {
            public Harmony_Patch()
            {
                try
                {
                    HarmonyInstance harmonyInstance = HarmonyInstance.Create("Lobotomy.NULL.EASYMOD");
            ...
            {
                overloadCount = (int)(overloadCount * 0.75);
                return true;
            }
            ...
            {
                __result = __instance.QliphothOverloadLevel >= 8;
            }
            ...
            public static void IsCleared_Malkut(ref bool __result)
            {
                ...
                __result = energy >= needEnery && CreatureOverloadManager.instance.GetQliphothOverloadLevel() >= 5;
            }
            ...
            public static void IsCleared_Chokhmah(ref bool __result)
            {
                float energy = EnergyModel.instance.GetEnergy();
                float needEnery = StageTypeInfo.instnace.GetEnergyNeed(PlayerModel.instance.GetDay());
                __result = energy >= needEnery && CreatureOverloadManager.instance.GetQliphothOverloadLevel() >= 8;
            }

            public static void IsCleared_Bina(BinahBossBase __instance, ref bool __result)
            {
                __result = __instance.Script.Phase == BinahBoss.BinahPhase.P3;
            }
            ...
            public static bool AddOverloadGague(CreatureOverloadManager __instance)
            {
                ...
                __instance.GetType().GetField("qliphothOverloadGauge", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(__instance, loadgauge + 1);
                return true;
            }
